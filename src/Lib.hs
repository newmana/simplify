...
douglasPeucker epsilon points
  ...
  | dmax > epsilon = douglasPeucker epsilon left >< douglasPeucker epsilon right
...
splitAtMaxDistance ls =
    D.foldlWithIndex (\(max, index) ni a -> if cp a ls > max then (cp a ls, ni) else (max, index)) (0.0, 0)
    where
